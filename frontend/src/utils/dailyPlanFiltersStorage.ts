import type { DailyPlanContextFilter } from "@/services/dailyPlanService";
...
export function buildDefaultDailyPlanFilters(): DailyPlanFiltersState {
...
const STORAGE_KEY = "fix-life-daily-plans-filters";

export interface DailyPlanFiltersState {
  context: DailyPlanContextFilter;
  startDate: string;
  endDate: string;
  selectedYear: number;
  selectedWeek: number;
...
function isDailyPlanContextFilter(value: unknown): value is DailyPlanContextFilter {
...
export function readDailyPlanFilters(fallback: DailyPlanFiltersState): DailyPlanFiltersState {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) {
      return fallback;
    }
    const parsed = JSON.parse(raw) as Partial<DailyPlanFiltersState>;
    return {
      context: isDailyPlanContextFilter(parsed.context) ? parsed.context : fallback.context,
...
export function writeDailyPlanFilters(state: DailyPlanFiltersState): void {
  try {
