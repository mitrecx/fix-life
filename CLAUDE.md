...
- 数据库服务不在本地, 需要通过 `ssh -f -N -L 6432:127.0.0.1:5432 josie@jo.mitrecx.top` 开启隧道后才能连接
- 通过 `ssh josie@jo.mitrecx.top` 连接服务器, 前后端应用都部署在这个服务器上
- 生产 HTTPS (`fixlife.mitrecx.top`) 使用 **Let's Encrypt + Certbot**（HTTP-01，webroot `/var/www/certbot`）；仓库参考配置见 `deploy/nginx.conf`，服务器为 `/etc/nginx/conf.d/00-fix-life.conf`。系统定时任务 `certbot-renew.timer` 负责续期；`/etc/sysconfig/certbot` 中 `POST_HOOK` 会在续期后执行 `systemctl reload nginx`。
