...
> **产品命名**：原「每日计划」强调事前规划，与「一切源自待办」的模型不符。已统一为 **「每日进度」**（见 §3.4）；REST 路径为 `/api/v1/daily-progress`（内部 ORM/表名 `daily_plans` 暂保留）。命名迁移详见 [每日进度命名统一](./2026-05-24-daily-progress-naming-migration-design.md)。
