...
Alembic 迁移 `20260525_2100_rename_daily_progress_tables`；只读兼容视图 `daily_plans` / `daily_tasks` 保留至 deprecate 窗口结束。API JSON 双字段：`daily_progress_day_id` + deprecated `daily_plan_id`，`daily_progress_entries` + deprecated `daily_tasks`。
...
- [x] DB 表 `daily_progress_days` / `daily_progress_entries`，列 `daily_progress_day_id`
- [x] 只读兼容视图 `daily_plans` / `daily_tasks`
- [x] API JSON 双字段 deprecate 窗口（`daily_plan_id` / `daily_tasks` 等）
...
| 0.5 | 2026-05-24 | Phase 5 DB 表/列重命名 + 只读兼容视图 |
